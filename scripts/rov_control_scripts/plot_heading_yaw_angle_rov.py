...
def plot_data():
    ...
    fig.canvas.set_window_title("plot_heading_yaw_angle_rov.py")
    fig.suptitle("")
    ax[0].set_title("First itteration of ROV heading - ROV heading angle")
    ax[0].set_ylabel("angle (deg)")
    ax[0].set_xlabel("simulation time (sec)")
    ax[0].grid()
    ax[0].legend(loc = "upper left")
    ax[1].set_title("Second itteration of ROV heading - ROV heading angle")
    ax[1].set_ylabel("angle (deg)")
    ax[1].set_xlabel("simulation time (sec)")
    ax[1].grid()
    ax[1].legend(loc = "upper left")

    ax[0].plot(time_list_first, heading_angle_list_list_first, color = "black", marker = "." , label = "")
    ax[1].plot(time_list_second, heading_angle_list_list_second, color = "green", marker = "." , label = "")
    fig.tight_layout()
...
if __name__ == "__main__":
    rospy.init_node("rov_heading_angle")

...
def pc_callback(pc_msg, pub):
    ...
    if len(points_list) >= rov:
        ...
        # rospy.loginfo(f"-----------------") 
        # #denne funker ikke+
        # rospy.loginfo(f"angle to center = {angle_to_center_ros} rad")  
        # rospy.loginfo(f"angle to center = {math.degrees(angle_to_center_ros)} deg") 

        # rospy.loginfo(points_list)
...
if __name__ == "__main__":
    ...
    # rospy.Subscriber("/box/depth_camera/points",PointCloud2,pc_callback, center_pub)
    rospy.Subscriber("/car/depth_camera/points",PointCloud2,pc_callback, center_pub)
